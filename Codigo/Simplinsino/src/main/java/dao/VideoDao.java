...
public class VideoDao extends DAO {
	...
	public boolean insert(Video video) {
		...
		try {
			String query = "INSERT INTO video (id_video, id_curso, link_video, descricao_video, nome_video) VALUES ('" +
					video.getId_video() + "', '" + video.getId_curso() + "', '" + video.getLink_video() + "', '" + video.getDescricao_video() + "', '" + video.getNome_video() + "');";
	...
	public List<Video> get(){
		...
		try {
			...
			while(rs.next()) {
				Video video = new Video(rs.getInt("id_video"), rs.getInt("id_curso"), rs.getString("link_video"), rs.getString("descricao_video"), rs.getString("nome_video"));
				videos.add(video);
			}
	...
	public Video getVideo(int id_video) {
		...
		try {
			...
			if(rs.next()) {
				...
				video.setId_curso(rs.getInt("id_curso"));
